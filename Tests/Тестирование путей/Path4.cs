...
namespace Tests
{
    // 4) Новая заявка -> Оплата регистрации -> Регистрация -> Оплата первичной экспертизы -> Ожидание первичной экспертизы -> Ожидаем завершения другого проекта -> Первичная Экспертиза -> Одобрено -> Оплата вторичной экспертизы -> Ожидание вторичной экспертизы -> Вторичная экспертиза -> Отклонено
    [TestClass]
    ...
    {        
        ...
        public void Run()
        {
            ...
            Assert.IsNull(currentApplication.expertise);
            ...
            //Освобождаем остальных сотрудников для вторичной экспертизы
            ...
            //Ожидание завершения другого проекта - не должно быть, так как все сотрудники свободны     
